...
def image_to_audio(image_path: str, output_path: str,
    ...
    # cv2.resize 参数是 (width, height)，输出 shape 是 (height, width) = (n_freq_bins, n_frames)
    S = cv2.resize(gray_flipped, (n_frames, n_freq_bins),
                   interpolation=cv2.INTER_LINEAR).astype(np.float32) / 255.0
    audio = librosa.griffinlim(S, n_iter=32, hop_length=hop_length, win_length=n_fft, n_fft=n_fft)
